...
export const PolicyPage = () => {
    ...
    const trimmedSearch = location.search.slice(1);
    let disabled = false;

    // Decode the URI component
    const decodedSearch = decodeURIComponent(trimmedSearch);

    // If there's extra equal sign at the end because of the format, you might want to remove it.
    const cleanSearch = decodedSearch.endsWith('=')
        ? decodedSearch.slice(0, -1)
        : decodedSearch;

    // Parse the JSON
    const queryParams = cleanSearch ? JSON.parse(cleanSearch) : {};

    const model = queryParams?.model || {};
    const vector = queryParams?.vector || {};
    const temp = queryParams?.temperature || '0';
    const storage = queryParams?.storage || {};
    const queryCount = queryParams?.queryCount || '3';
    ...
    const [selectedModel, setSelectedModel] = useState<Model>(model as Model);
    ...
    const [selectedStorage, setSelectedStorage] = useState<Model>(
        storage as Model,
    );
    ...
    const [limit, setLimit] = useState<number>(parseInt(queryCount));
    const [temperature, setTemperature] = useState<number>(parseInt(temp));
    ...
    const ask = async (questionText: string) => {
        try {
            ...
            setMessages((prev) => [
                ...prev,
                { type: 'user', content: questionText },
            ]);
            ...
            if (selectedVectorDB && selectedVectorDB.database_id) {
                ...
                for (let i = 0; i < outputArray.length - 1; i++) {
                    if (outputArray[i]) {
                        const content =
                            typeof outputArray[i] === 'object' &&
                            outputArray[i] !== null
                                ? outputArray[i].content ||
                                  outputArray[i].Content
                                : '';
            ...
            const contextDocsString =
                context_docs.length > 0
            ...
            const errorMessage =
                e instanceof Error
                    ? e.message
                    : 'There is an error, please check pixel calls';
            setError(errorMessage);
    ...
    useEffect(() => {
        ...
        actions.run(pixel).then((response) => {
            ...
            if (Array.isArray(output)) {
                ...
                setSelectedModel((prev) =>
                    prev?.database_id ? prev : output[2],
                );
            }
        });
        ...
        actions.run(pixel).then((response) => {
            ...
            if (Array.isArray(output)) {
                ...
                setSelectedVectorDB((prev) =>
                    prev?.database_id ? prev : output[10],
                );
                setRefresh(false);
            }
        });
        ...
        actions.run(pixel).then((response) => {
            ...
            if (Array.isArray(output)) {
                ...
                setSelectedStorage((prev) =>
                    prev?.database_id ? prev : output[1],
                );
            }
        });
    ...
    return (
        <Container>
                ...
                <ChatHeaderContainer>
                    ...
                    <HeaderControls>
                        <ModelLabel>
                            ...
                            <span>
                                {selectedModel?.database_name || 'Model'}
                            </span>
                        </ModelLabel>
                    ...
                    {messages.length === 0 ? (
                        <>
                            <EmptyChat>
                                ...
                                <EmptyChatText>
                                    Use the chat settings in the upper righthand
                                    corner to tailor the conversation to fit
                                    your needs.
                                </EmptyChatText>
                            </EmptyChat>
                            {/* <ExampleGrid>
                ...
                <InputContainer>
                    <InputWrapper>
                        ...
                        <MessageTextarea
                            ...
                            placeholder={
                                attachedFile
                                    ? 'Click send to upload file or add a message'
            ...
            {sideOpen && <SettingsOverlay onClick={() => setSideOpen(false)} />}
